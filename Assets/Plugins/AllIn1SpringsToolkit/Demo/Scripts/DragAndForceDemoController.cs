...
namespace AllIn1SpringsToolkit.Demo.Scripts
{
    public class DragAndForceDemoController : DemoElement
    {
        ...
        private void UpdateForceAndDrag()
        {
            ...
            robotSpringComponent.positionSpring.unifiedForce = Mathf.Lerp(forceRange.x, forceRange.y, forceSlider.value);
            robotSpringComponent.positionSpring.unifiedDrag = Mathf.Lerp(dragRange.x, dragRange.y, dragSlider.value);

            forceText.text = robotSpringComponent.positionSpring.unifiedForce.ToString("F0");
            dragText.text = robotSpringComponent.positionSpring.unifiedDrag.ToString("F0");
        }
        ...
        {
            float velocityX = robotSpringComponent.positionSpring.GetVelocity().x * scaleMotionMultiplier;
            ...
            TextMeshProUGUI newForceText, TextMeshProUGUI newDragText)
        {
            ...
            forceSlider.value = RemapUnclamped(robotSpringComponent.positionSpring.unifiedForce, forceRange.x, forceRange.y, 0f, 1f);
            dragSlider.value = RemapUnclamped(robotSpringComponent.positionSpring.unifiedDrag, dragRange.x, dragRange.y, 0f, 1f);
